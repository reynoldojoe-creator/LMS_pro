...
@app.get("/vetting/pending")
async def get_pending_batches(db: Session = Depends(get_db)):
    """
    Returns a list of 'batches' (Rubrics) that have questions needing review.
    We'll treat each Rubric as a batch.
    """
    # 1. Get all rubrics that have associated questions
    # Ideally, we filter only those with pending questions, but showing all active batches is fine
    rubrics = db.query(database.Rubric).order_by(database.Rubric.created_at.desc()).all()
    
    batches = []
    for r in rubrics:
        # Calculate stats
        total = db.query(database.Question).filter(database.Question.rubric_id == r.id).count()
        if total == 0:
        ...
        batches.append({
            ...
            "subjectId": str(r.subject_id),
            "title": r.title,
            "facultyName": "Faculty User", # Placeholder as we don't have user links yet
            "totalQuestions": total,
            "reviewedQuestions": reviewed,
            "approvedCount": approved,
            "rejectedCount": rejected,
            "quarantinedCount": quarantined,
            "generatedAt": r.created_at.isoformat() if r.created_at else datetime.now().isoformat(),
            "status": status,
...
async def get_batches(status: Optional[str] = None, db: Session = Depends(get_db)):
    """
    List batches (rubrics) with optional status filtering.
    """
    # Reuse logic from get_pending_batches (or extraction common logic)
    # For now, duplicating logic for safety and speed
    rubrics = db.query(database.Rubric).order_by(database.Rubric.created_at.desc()).all()
    ...
    for r in rubrics:
        ...
        pending = db.query(database.Question).filter(database.Question.rubric_id == r.id, database.Question.status == "pending").count()
        approved = db.query(database.Question).filter(database.Question.rubric_id == r.id, database.Question.status == "approved").count()
        rejected = db.query(database.Question).filter(database.Question.rubric_id == r.id, database.Question.status == "rejected").count()
        quarantined = db.query(database.Question).filter(database.Question.rubric_id == r.id, database.Question.status == "quarantined").count()
...
async def get_batch_details(batch_id: int, db: Session = Depends(get_db)):
    ...
    quarantined = sum(1 for q in questions if q.status == "quarantined")
    reviewed = approved + rejected + quarantined
    ...
    for q in questions:
        formatted_questions.append({
            "id": str(q.id),
            "subjectId": str(q.subject_id),
            "topicId": str(q.topic_id) if q.topic_id else "",
            "loId": q.lo_id or "",
            "type": q.question_type or "short_answer",
    ...
    return {
        ...
        "rejectedCount": rejected,
        "quarantinedCount": quarantined,
        "generatedAt": rubric.created_at.isoformat() if rubric.created_at else datetime.now().isoformat(),
        "status": status,
        "questions": formatted_questions
    }

@app.post("/vetting/{question_id}/approve")
async def approve_question(question_id: int, db: Session = Depends(get_db)):
...
@app.post("/vetting/{question_id}/reject")
async def reject_question(question_id: int, payload: Dict[str, Any], db: Session = Depends(get_db)):
    question = db.query(database.Question).filter(database.Question.id == question_id).first()
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
        
    question.status = "rejected"
    question.rejection_reason = payload.get("reason", "No reason provided")
    db.commit()
    return {"message": "Question rejected", "status": "rejected"}

@app.post("/vetting/{question_id}/quarantine")
async def quarantine_question(question_id: int, payload: Dict[str, Any], db: Session = Depends(get_db)):
    question = db.query(database.Question).filter(database.Question.id == question_id).first()
    if not question:
        raise HTTPException(status_code=404, detail="Question not found")
        
    question.status = "quarantined"
    # Append notes to rejection_reason or use a separate field if we had one
    # Reusing rejection_reason for notes for now
    question.rejection_reason = f"QUARANTINE_NOTES: {payload.get('notes', '')}"
    db.commit()
    return {"message": "Question quarantined", "status": "quarantined"}
